...
namespace GroundWellDesign
{
    public partial class Document : Window
    {
        public Document()
        {
            ...
            dibiao.LayerParams.YanXing = YanXingOpt[0];
            layers.Add(dibiao);
            caiDongComBox.SelectedIndex = 0;

            //向导式录入初始化
            editLayer = new LayerBaseParamsViewModel(this);
            guideBind(editLayer);
            ...
            //水泥环增益初始化
            editZengYi = new ZengYiParamsViewModel(this);

            //人工设计
            manuDesignParams = new ManuDesignParamsViewModel(this);
        ...
        //向导式绑定
        private void guideBind(LayerBaseParamsViewModel editLayer)
        {
        ...
        private void initialData()
        {
            ...
            for (int i = 0; i < 89; i++)
            {
                ...
                layers.Add(param);
                param.LayerParams.YanXing = yanxings[i];
                param.LayerParams.CengHou = data2[i * 10];
                param.LayerParams.ZiRanMiDu = data2[i * 10 + 1];
                param.LayerParams.BianXingMoLiang = data2[i * 10 + 2]; ;
                param.LayerParams.KangLaQiangDu = data2[i * 10 + 3];
                param.LayerParams.KangYaQiangDu = data2[i * 10 + 4];
                param.LayerParams.TanXingMoLiang = data2[i * 10 + 5];
                param.LayerParams.BoSonBi = data2[i * 10 + 6];
                param.LayerParams.NeiMoCaJiao = data2[i * 10 + 7];
                param.LayerParams.NianJuLi = data2[i * 10 + 8];
                param.LayerParams.Q0 = data2[i * 10 + 9];
            }
        }
